...
def split_raster(input_tif, output_dir, tile_size=128):
    ...
    region_name = os.path.basename(input_tif).replace("planet_", "").replace("_clip_normalized.tif", "")
...
# 複数の地域をリストで指定
regions = ["gofukuya", "iburi", "notoEarth", "notoRain"]  # 地域名のリスト（必要に応じて追加）

# 出力ディレクトリ
output_dir = "/media/hina/AE60-1503/sabo_UNet_Planet_trial03/tiles/features/"

# 各地域に対してタイル分割実行
for region in regions:
    input_tif = f"/media/hina/AE60-1503/sabo_UNet_Planet_trial03/features/planet_{region}_clip_normalized.tif"
